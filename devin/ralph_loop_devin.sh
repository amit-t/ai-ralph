...
main() {
    ...
    execute_devin_session 1 "$work_dir"
    local exec_result=$?
        ...
        if [[ "$WORKTREE_ENABLED" == "true" ]] && worktree_is_active; then
            ...
            while [[ $gate_result -ne 0 && $qg_attempt -lt $MAX_QG_RETRIES ]]; do
                ...
                if [[ -n "$(cd "$work_dir" && git status --porcelain 2>/dev/null)" ]]; then
                    (cd "$work_dir" && git add -A && git commit -m "ralph-devin: pre-QG-retry auto-commit (attempt $qg_attempt)") 2>/dev/null || true
                fi

                # Re-invoke Devin with the QG fix prompt
...
- DO NOT navigate to or modify files outside this directory."
                ...
                build_devin_command "$qg_prompt_file" "" "" "true" "$qg_worktree_directive"

                log_status "INFO" "QG fix command: ${DEVIN_CMD_ARGS[*]:0:5}..."
                local qg_timestamp
                qg_timestamp=$(date '+%Y-%m-%d_%H-%M-%S')
                local qg_output_file="$LOG_DIR/devin_qg_fix_${qg_timestamp}.log"

                set +e
                (cd "$work_dir" && "${DEVIN_CMD_ARGS[@]}" > "$qg_output_file" 2>&1)
