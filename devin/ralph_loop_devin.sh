...
# Default Devin model for the Ralph loop. Hardcoded to swe-1.6 (project-level
...
DEVIN_MODEL="${DEVIN_MODEL:-swe-1.6}"
...
# spurious .ralph/ in whatever directory ralph-devin happens to start in.
...
Options:
    ...
    --model MODEL           Set Devin model: swe-1.6, opus, sonnet, swe, gpt (default: swe-1.6)
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    # what produced spurious .ralph/ stubs when ralph-devin (or a read-only
    # alias like rpd.status) was invoked from a workbench root.
    if [[ "$WORKSPACE_MODE" != "true" && -z "${CONTINUOUS_WORKER_ID:-}" ]] && ! _ralph_dir_is_valid_target; then
        ...
        echo "       Run 'ralph-enable-devin' here first, or cd into your workspace." >&2
        exit 1
    fi
