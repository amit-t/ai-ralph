...
# Ralph Enable for Devin CLI - Interactive Wizard for Existing Projects
# Adds Ralph+Devin configuration to an existing codebase
# Parallel to ralph_enable.sh (Claude Code)
#
# Usage:
#   ralph-devin-enable              # Interactive wizard
#   ralph-devin-enable --from beads # With specific task source
#   ralph-devin-enable --force      # Overwrite existing .ralph/
#   ralph-devin-enable --skip-tasks # Skip task import
#
# Version: 0.1.0
...
if [[ -f "$SCRIPT_DIR/lib/devin_adapter.sh" ]]; then
    RALPH_DIR=".ralph"
    source "$SCRIPT_DIR/lib/devin_adapter.sh"
elif [[ -f "$RALPH_HOME/devin/lib/devin_adapter.sh" ]]; then
    RALPH_DIR=".ralph"
    source "$RALPH_HOME/devin/lib/devin_adapter.sh"
fi
...
show_help() {
    cat << EOF
Ralph Enable for Devin CLI - Add Ralph+Devin to Existing Projects

Usage: ralph-devin-enable [OPTIONS]
...
Examples:
    ...
    ralph-devin-enable --from beads
    ralph-devin-enable --from github --label "ralph-task"
    ralph-devin-enable --from prd --prd ./docs/requirements.md
    ralph-devin-enable --skip-tasks
    ralph-devin-enable --force
...
What this command does:
    ...
    6. Creates .ralphrc.devin with Devin-specific settings (separate from Claude's .ralphrc)

This command is the Devin CLI equivalent of 'ralph-enable'.

EOF
}

# =============================================================================
# DEVIN-SPECIFIC RALPHRC GENERATION
# =============================================================================

generate_devin_ralphrc() {
...
# Generated by: ralph-devin-enable
...
# DEVIN_MAX_ACU=             # Maximum ACU limit per session
...
run_wizard() {
    echo ""
    echo "╔════════════════════════════════════════════════╗"
    echo "║   Ralph Enable for Devin CLI - Setup Wizard   ║"
    ...
    # Check Devin CLI
    echo ""
    echo "  Checking Devin CLI..."
    if command -v devin &>/dev/null; then
        echo "  ✅ Devin CLI found"
    else
        echo "  ⚠️  Devin CLI not found. Install: pip install devin-cli"
        echo "  The project will still be configured, but you'll need devin-cli to run ralph-devin"
    fi
...
AGENTEOF
    ...
    echo "  ✅ Generated .ralphrc.devin (Devin CLI)"
    ...
    for file in .ralph/PROMPT.md .ralph/fix_plan.md .ralph/AGENT.md .ralphrc.devin; do
        if [[ -f "$file" ]]; then
            echo "  ✅ $file"
        else
            echo "  ❌ $file (missing)"
    ...
    if [[ "$all_ok" == "true" ]]; then
        echo "╔════════════════════════════════════════════════╗"
        echo "║   ✅ Ralph+Devin enabled successfully!        ║"
        echo "╚════════════════════════════════════════════════╝"
        echo ""
        echo "Next steps:"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        -h|--help)
            ...
            ;;
        -v|--version)
            echo "ralph-devin-enable version $VERSION"
            exit 0
            ;;
        --from)
            TASK_SOURCE="$2"
