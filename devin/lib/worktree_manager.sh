...
# worktree_manager.sh - Git worktree lifecycle management for Ralph Devin
...
worktree_auto_commit() {
    local message="${1:-ralph-devin: auto-commit work}"
    local workdir="$_WT_CURRENT_PATH"
...
worktree_merge() {
    ...
    if [[ "$WORKTREE_AUTO_COMMIT" == "true" ]]; then
        worktree_auto_commit "ralph-devin: auto-commit from ${branch}" 2>/dev/null || true
    fi
    ...
    case "$strategy" in
        squash)
            ...
            if [[ $merge_exit -eq 0 ]]; then
                git commit -m "ralph-devin: squash merge from ${branch} ($ahead_count commits)" 2>/dev/null
                merge_exit=$?
            fi
            ;;
        merge)
            git merge --no-ff "$branch" -m "ralph-devin: merge from ${branch}" 2>/dev/null
            merge_exit=$?
            ;;
        rebase)
...
worktree_cleanup() {
    ...
    # Sync .ralph state back to main project (fix_plan.md may have been updated by devin)
