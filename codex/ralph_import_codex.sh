...
# Ralph Import for Codex CLI - Convert PRDs to Ralph format using Codex
# Parallel to ralph_import.sh (Claude Code) — uses Codex sessions for conversion
#
# Version: 0.1.0
...
# shellcheck disable=SC2034 # consumed by sourced codex_adapter.sh
...
# Temporary files
CONVERSION_OUTPUT_FILE=".ralph_codex_conversion_output.json"
...
show_help() {
    cat << EOF
Ralph Import for Codex CLI - Convert PRDs to Ralph format

Usage: ralph-codex-import [OPTIONS] <prd-file> [project-name]
...
Options:
    ...
    --timeout MIN   Set Codex session timeout (default: $CODEX_TIMEOUT_MINUTES)
...
Examples:
    ralph-codex-import product-requirements.md my-app
    ralph-codex-import requirements.txt webapp
    ralph-codex-import --print api-spec.json

EOF
}

# Parse arguments
# shellcheck disable=SC2034 # CLI flag placeholder; reserved for non-interactive print mode (see --print case)
PRINT_MODE=false
PRD_FILE=""
PROJECT_NAME=""
...
# Create Codex session for conversion
log "INFO" "Creating Codex session for PRD conversion..."
local_output_file="$PROJECT_NAME/$CONVERSION_OUTPUT_FILE"

timeout_seconds=$((CODEX_TIMEOUT_MINUTES * 60))

session_id=$(codex_create_session "$CONVERSION_PROMPT" "Ralph Import: $PROJECT_NAME" "" "")
create_exit=$?

if [[ $create_exit -ne 0 || -z "$session_id" ]]; then
    log "ERROR" "Failed to create Codex session for conversion"
    exit 1
fi

log "SUCCESS" "Codex session created: ${session_id:0:20}..."
log "INFO" "Waiting for conversion to complete..."

# Poll for completion
codex_poll_session "$session_id" "$local_output_file" "$timeout_seconds" ""
poll_exit=$?

if [[ $poll_exit -ne 0 ]]; then
    log "ERROR" "Codex conversion session failed or timed out"
    log "INFO" "You can check the session at: codex open $session_id"
    exit 1
fi

log "SUCCESS" "Codex conversion completed"

# Parse output and create files
output_content=$(codex_extract_result_text "$local_output_file")
...
# Generate Codex-specific .ralphrc.codex (separate from Claude's .ralphrc)
cat > "$PROJECT_NAME/.ralphrc.codex" << RALPHRCEOF
# .ralphrc.codex - Ralph project configuration (Codex CLI)
# Generated by: ralph-codex-import
...
git add .
git commit -m "Initial Ralph+Codex project from PRD import"
cd ..

# Clean up temp files
rm -f "$local_output_file"

log "SUCCESS" "Project '$PROJECT_NAME' created with Codex CLI support!"
echo ""
echo "Next steps:"
