...
# Ralph Monitor for Codex CLI - Live Status Dashboard
# Parallel to ralph_monitor.sh (Claude Code) — adapted for Codex session monitoring
#
# Version: 0.1.0
...
Usage: ralph-codex-monitor [OPTIONS]
...
if [[ ! -d "$RALPH_DIR" ]]; then
    ...
    echo "Run 'ralph-codex-enable' or 'ralph-codex-setup' first"
    exit 1
fi
...
while true; do
    ...
    echo -e "${BOLD}${PURPLE}║         Ralph Monitor (Codex CLI Engine)             ║${NC}"
    echo -e "${BOLD}${PURPLE}╚══════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    if [[ -f "$STATUS_FILE" ]]; then
        ...
        codex_session=$(echo "$local_status" | jq -r '.codex_session_id // ""' 2>/dev/null || echo "")
        next_reset=$(echo "$local_status" | jq -r '.next_reset // ""' 2>/dev/null || echo "")
        timestamp=$(echo "$local_status" | jq -r '.timestamp // ""' 2>/dev/null || echo "")
        engine=$(echo "$local_status" | jq -r '.engine // "codex"' 2>/dev/null || echo "codex")
        ...
        if [[ -n "$codex_session" ]]; then
            echo -e "  ${BOLD}Session:${NC}    ${codex_session:0:30}..."
        fi
        ...
        echo -e "  ${YELLOW}No status file found. Ralph Codex may not be running.${NC}"
    fi
    ...
    # Codex session info
    if [[ -f "$RALPH_DIR/.codex_session_id" ]]; then
        codex_sid=$(cat "$RALPH_DIR/.codex_session_id" 2>/dev/null)
        if [[ -n "$codex_sid" ]]; then
            echo -e "  ${BOLD}Codex Session ID:${NC} ${CYAN}${codex_sid}${NC}"
        fi
    fi
