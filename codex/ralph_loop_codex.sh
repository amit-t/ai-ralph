...
# spurious .ralph/ in whatever directory ralph-codex happens to start in.
...
setup_tmux_session() {
    local session_name
    session_name="ralph-codex-$(date +%s)"
...
main() {
    ...
    if [[ $exec_result -eq 0 ]]; then
        ...
        _fail_session_id=$(cat "$CODEX_SESSION_FILE" 2>/dev/null || echo "")
        if [[ -z "$_fail_session_id" ]]; then
            _fail_session_id=$(codex_get_latest_session_id 2>/dev/null || echo "")
        fi
        if worktree_is_active; then
        ...
        log_status "ERROR" "Codex execution failed (exit $exec_result)"
        exit 1
    fi
}
...
# Usage: ralph-codex --workspace [--parallel N]
#
run_workspace_mode() {
    # Load project-specific configuration from .ralphrc.codex
    if load_ralphrc; then
        [[ "$RALPHRC_LOADED" == "true" ]] && log_status "INFO" "Loaded configuration from .ralphrc.codex"
    fi
    ...
    log_status "SUCCESS" "Ralph Codex workspace mode starting"
    ...
    # Execute Codex session with repo directory as work_dir.
    set +e
    execute_codex_session 1 "$work_dir" "$task_id" "$line_num" "$task_desc"
    local exec_result=$?
    set -e
    ...
    if [[ $exec_result -eq 0 ]]; then
        ...
        if [[ $files_changed -eq 0 ]]; then
            ...
            log_status "SUCCESS" "Ralph Codex workspace complete (no changes)."
            return 0
        fi
        ...
        log_status "SUCCESS" "Ralph Codex workspace task complete: [$repo_name] $task_desc"
    else
...
#
# Each task gets: worktree isolation → Codex execution → quality gates → PR → cleanup
#
# Args:
...
_workspace_execute_task() {
    ...
    # Execute Codex session with the repo as work directory.
    execute_codex_session 1 "$work_dir" "$task_id" "" "$task_desc"
    local result=$?
...
run_continuous_workspace() {
    ...
    if [[ "${NO_TABS:-false}" != "true" ]] && tabs_supported_by_terminal; then
        ...
        export RALPH_TABS_ENGINE_CMD="${RALPH_CMD_PREFIX:-}ralph-codex"
...
# _singlerepo_execute_task — single-repo executor wrapping execute_codex_session
...
Continuous Parallel Execution:
    ...
    Examples:
        ralph-codex --parallel 2 10               # single-repo, 2 workers, 10 attempts (tabs by default)
        ralph-codex --parallel 2 10 --no-tabs     # same, but force single-pane
        ralph-codex --workspace --parallel 2 10   # multi-repo workspace, same shape
...
Examples:
    ...
    ralph-codex --task R05             # Execute task **R05** by its ID
    ralph-codex --task 5 --no-codex-auto-exit  # Interactively work on task 5
    ralph-codex --workspace               # Run workspace mode (multi-repo)
    ralph-codex --workspace --parallel 3  # Parallel workspace (3 repos simultaneously)
...
Bash Aliases (rpx):
    ...
    rpx.ws           # Workspace mode (multi-repo)
    rpx.ws.p 3       # Parallel workspace (3 repos)
...
if [[ "$CONTINUOUS_MODE" == "true" ]]; then
    ...
    if [[ "$CODEX_AUTO_EXIT" == "false" ]]; then
        echo "Error: --no-codex-auto-exit is incompatible with continuous mode (--parallel N M)."
        echo "       Continuous mode requires Codex to auto-exit so each worker can complete and respawn."
        exit 1
    fi
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    # what produced spurious .ralph/ stubs when ralph-codex (or a read-only
    # alias like rpx.status) was invoked from a workbench root.
    if [[ "$WORKSPACE_MODE" != "true" && -z "${CONTINUOUS_WORKER_ID:-}" ]] && ! _ralph_dir_is_valid_target; then
        ...
        echo "       Run 'ralph-enable-codex' here first, or cd into your workspace." >&2
        exit 1
    fi
